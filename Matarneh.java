...
public class Matarneh {
	...
	public void calculateQ(){
		int[] threads = new int[hash.size()];
		int index=0;
		for(int key:hash.keySet()){
			threads[index]=hash.get(key);
			index++;
		}
		Arrays.sort(threads);
		int length = threads.length;
		if(length%2==0&&length!=0){
			q = (threads[length/2]+threads[(length/2)-1])/2;
		}
		else {
			q = (threads[(length-1)/2]);
		}
	...
	public static void main(String[] args) throws IOException {
		Matarneh b = new Matarneh("C:/Users/phoebe/Desktop/testl.txt");
		b.calculate();
